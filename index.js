...
module.exports	=	class {
	...
	*													multi	-	发送二进制信息
	*													null	-	不写明发送信息格式
	* @param	{array}	options.acceptEncoding			允许返回数据格式
	*													gzip	-	gzip压缩格式
	*													defalte	-	defalte压缩格式
	...
	*/
	static request( url, type = "GET", data = {}, options = {} ){
		...
		if( type === "GET" ){
			...
			if( url.query === undefined || url.query === null ){
		...
		if( options.acceptEncoding ){
			headers[ "Accept-Encoding" ]	=	options.acceptEncoding.join( "," );
		}
		httpOptions = {
		...
		if( options.pfx ){
			httpOptions.pfx		=	options.pfx;
			if( options.passphrase ){
				httpOptions.passphrase	=	options.passphrase;
			}
		}
		if( url.port !== null ){
		...
		return new Promise( ( resolve, reject ) => {
			let request = httpClient.request( httpOptions, response => {
